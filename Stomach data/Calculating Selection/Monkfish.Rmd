...
The diet PPMR actually is less consisten across pred sizes.
...
This looks very hartd to model. Maybe 3 gaussians?
...
This doesnt shift well. WIll try shift from biomass to number.
...
Its not awful, but its not great. I will try more gaussians.


```{r}

spratnu <- repeat_elements(log(sprat$ppmr), sprat$nprey_perpred*sprat$prey_ind_weight_g^dig)
gmm <- densityMclust(spratnu, G=5)
dplot <- data.frame(x=gmm[["data"]], density=gmm[["density"]])
...
Still a pretty terrible fit. I can try exponential, I doubt it.
...
I would say its a better fit. IT is still not that good.
