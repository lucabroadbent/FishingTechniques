...
When looking at the contribution to the diet, there is variation around 
a PPMR value, but this might be to do with the groupings of the pred weights,
so I think it is safe to say that PPMR is the same across all pred sizes.
...
Not sure which fit would be best here. a mixture gaussian model with 2 gaussians
will fit well.

The ecology of norway pout is a mixture of plantivore and piscovore. Therefore
it might make sense there are 2 feeding modes, one for filtering, the other
...
I am going to check now that there aren't any problem datapoints

```{r}

ggplot()+
  geom_point(data=sprat, aes(x=nprey_perpred, y=log(ppmr)))

#There is one point at around 12, which is a bit of an outlier. I will remove it.
#it changes the gaussians fitted too much.

sprat <- sprat%>%filter(nprey_perpred<600)
...
This is not bad, this works well. I will shift it over now.
...
It doesn't shift over very well. Will fit to the biomass
...
This also doesnt shify very well. Should I try the exponential?
...
ggplot(stomach) + 
  geom_density(aes(l, weight=weight_numbers), fill="lightblue")+
  geom_density(aes(l, weight=biomass), fill="#ffcccb")+
 xlab("Log of predator/prey mass ratio") + 
...
ggplot(stomach) + 
 ...
 xlab("Log of predator/prey mass ratio") + 
  ...
  geom_line(aes(l, Density), data = shiftnumberdist, color = "red")+
...
When fitted to biomass, and shifted to number, it is good enough.
