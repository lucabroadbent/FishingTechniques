...
This section is about using a different way of fitting normal distribution, to see if it works better
for when we calcualate our own individual prey weight column, instead of using the one in the model 

```{r}
#filtering data out, and calculating our own column

restom <- stom_df |>
    filter(!is.na(pred_weight_g),
           prey_ind_weight_g > 0,
           prey_count > 0) |>
    select(pred_species, pred_weight_g, prey_weight_g, prey_count) |>
    group_by(pred_species) |> 
    filter(n() > 1000) |> 
    rename(w_prey = prey_weight_g, w_pred = pred_weight_g, n_prey = prey_count) |>
    mutate(log_ppmr = log(w_pred / (w_prey/n_prey)),
           weight_numbers = n_prey / sum(n_prey),
           w_prey=w_prey/n_prey)

#now we are calculating the new means and sd.

dig <- 2/3
weight <- restom$n_prey * (restom$w_prey / restom$w_pred)^dig
weight <- weight / sum(weight)

est_mean <- weighted.mean(restom$log_ppmr, weight)

est_mean_num <- weighted.mean(restom$log_ppmr, restom$n_prey)
est_sd_alt <- sqrt((est_mean_num - est_mean) / dig)
```
```{r}
#now we want to replot it all. Firstly for the diet.

grid <- seq(0, max(restom$log_ppmr), length = 100)
normaldens <- plyr::ddply(restom, "pred_species", function(stomach) {
    weight <- restom$n_prey * (restom$w_prey / restom$w_pred)^dig
    weight <- weight / sum(weight)
    ...
    data.frame( 
        log_ppmr = grid,
        density = dnorm(grid, est_mean, est_sd_alt)
        )
})
...
#ok this doesn't fit very well - the standard deviation is way too large
```
Ok, so once we use our own column for prey weight, as theirs is not calculated correctly, 
or it is calculated differently (and we dont know how), the normal distributions that we 
have been fitting do not work very well. 
