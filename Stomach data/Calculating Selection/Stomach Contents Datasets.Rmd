...
```
 ...
 the difference should be 2/3 (I am also not sure why this is)
 ...
 distribution for the biomass density by -2/3.
...
dist <- dtexp(grid, alpha = (biomassestco[1]), ll = biomassestco[2], ul = biomassestco[3], lr = biomassestco[4], ur = biomassestco[5])
biomassdist <- data.frame(l=grid, Density=dist)
dist <- dtexp(grid, alpha = (biomassestco[1]+2/3), ll = biomassestco[2], ul = biomassestco[3], lr = biomassestco[4], ur = biomassestco[5])
numberdist <- data.frame(l=grid, Density=dist)
#now plot these two together
...
gmm2 <- gmm
gmm2[["parameters"]][["mean"]] <- gmm2[["parameters"]][["mean"]]-(2/3)*gmm2[["parameters"]][["variance"]][["sigmasq"]]
...
shifted_fit[["parameters"]][["mean"]] <- shifted_fit[["parameters"]][["mean"]]+(2/3)*shifted_fit[["parameters"]][["variance"]][["sigmasq"]]
